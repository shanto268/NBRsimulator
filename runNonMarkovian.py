...
import numpy as np
from TrappingSimulator_nonmarkov import QPtrapper
...
args = {'N':N,'Lj':Lj,'xne':xne,'tauRelease':tauRelease,'tauCommon':tauCommon,'tauRare':tauRare,
        'tauRecomb':tauRecomb,'sampleRate':sampleRate,
             'phi':phi,'Delta':2.72370016e-23,'T':0.025,'memory':memory}
...
plotComplexHist(ax1,res.signal.real,res.signal.imag)
means = plt.ginput(-1)
...
plotTimeSeries(ax2,data,np.array(test.nTrapped),Q,time*1000,0,10,zeroTime=False)
plt.suptitle(f'Non-markovian simulation | $\tau_m$ = {memory*1e6:.1f} $\mu$s')
...
print(f'there are {np.mean(test.nTrapped):.4f} trapped QPs on average')
...
countshmm, centershmm = plotTauDist(ax3,hmmdist,bins=80,color='blue',alpha=0.3)
dist = qp.extractTimeBetweenTrapEvents(test.nTrapped,time)
counts, centers = plotTauDist(ax3,dist,bins=80,color='grey',alpha=0.3)
plt.show(block=False)
...
countshmm2, centershmm2 = plotTauDistScaled(ax4,hmmdist,bins=80,color='blue',alpha=0.3)
counts2, centers2 = plotTauDistScaled(ax4,dist,bins=80,color='grey',alpha=0.3)
plt.show()
